...
// Pause song to prevent auto-play on page load
song.pause();
...
const generateNum = (max) => {
	/*
		...
		INPUT: max - the maximum number to generate
	*/
...
const changeTheme = (themeName, bypassChange) => {
	...
	*/
	...
	if (currentTheme != themeName || bypassChange === true) {
		...
		fetch(`json/themes/${themeName}.json`)
...
const eventCountdownLoop = () => {
	...
	*/
	...
	// If server-sync mode is enabled, use the server's time
...
const changeEvent = (eventsName, bypassChange = false) => {
	...
	*/
	...
	if (currentEvents != eventsName || bypassChange === true) {
		...
		fetch(`json/events/${eventsName}.json`)
		.then(response => response.json())
		.then(ajaxData => {
			try {
